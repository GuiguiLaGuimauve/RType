...
// Last update Fri Dec 23 02:02:37 2016 julien dufrene
//
  ...
  if (_user.size() != 0)
    {
      ...
      if (_user[0]->getStatus() == true && _user[0]->haveSomethingToRead() == true)
	{
	  PacketUnknown pk = _user[0]->popBufferRead();
	  _read->push(PacketC(pk, _user[0]));
